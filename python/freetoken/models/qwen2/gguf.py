"""Qwen2 GGUF adapter: build the FreeToken ``ModelConfig`` from GGUF metadata
and map GGUF tensors onto the qwen2 modules.

Dense model: no routed experts. The projections and the embedding stay in their
native packed block layout (``.qweight``); norms dequantize to bf16, attention
biases stay F32.
"""
...
def _require_tp1(what: str) -> None:
    ...
    if get_tp_info().size != 1:
        raise NotImplementedError(
            f"qwen2 GGUF {what} currently supports TP=1 only "
    ...
    *,
    ...
    for t in iter_gguf_tensors(model_path):
        ...
        if suffix == "attn_q.weight":
            ...
            raise ValueError(f"unmapped qwen2 GGUF tensor: {name}")
...
def convert_qwen2_to_gguf(model, config: ModelConfig) -> None:
...
__all__ = ["parse_gguf_config", "iter_gguf_weights", "is_gguf_model", "convert_qwen2_to_gguf"]
